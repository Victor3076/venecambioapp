...
export default function ManualCuadrePage() {
    const [egli, setEgli] = useState<BalanceRow>({ yesterday: "0", today_pass: "0", today_clps: "0", total: 0 })
    const [vicmar, setVicmar] = useState<BalanceRow>({ yesterday: "0", today_pass: "0", today_clps: "0", total: 0 })
    const [corriente, setCorriente] = useState<BalanceRow>({ yesterday: "0", today_pass: "0", today_clps: "0", total: 0 })
    const [cyber, setCyber] = useState<BalanceRow>({ yesterday: "0", today_pass: "0", today_clps: "0", total: 0 })
    const [manualCuadre, setManualCuadre] = useState("0")

    const calculateTotal = (row: BalanceRow) => {
    ...
    const BalanceBlock = ({ title, data, onChange, color = "primary" }: { title: string, data: BalanceRow, onChange: (d: BalanceRow) => void, color?: string }) => (
    ...
    return (
        <div className="space-y-6 max-w-4xl mx-auto pb-10">
            ...
            <div className="bg-yellow-500/10 border border-yellow-500/20 p-4 rounded-xl flex items-start gap-4">
                <div className="bg-yellow-500 text-white p-1 rounded-full mt-0.5">
                ...
                <div className="text-sm">
                    ...
                    <p className="text-yellow-700/80">Este cuadre es una herramienta de cálculo temporal y **no guarda datos** en el servidor. Al actualizar la página, los valores se reiniciarán. Úsalo para verificar tus cierres antes de registrarlos oficialmente.</p>
                </div>
            </div>
        </div>
    )
